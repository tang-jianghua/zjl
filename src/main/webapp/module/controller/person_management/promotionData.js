App.controller("promotionDataCtrl",function($scope, $rootScope, $location, $http, $compile, path, $stateParams, $sce, publicService) {
							...
							if (operation == "read") {
								var parameter = {
									...
									param:{
										...
										principal:$scope.search.principal,
									}	
								};
					...
					parameterMap : function(options, operation) {
						if (operation == "read") {
							...
							if(options.sort!=null && options.sort.length>0 ){
								if(options.sort[0].field=="totalprice"){
									options.sort[0].field="temp.total_price"
								}
							...
							var parameter = {
								...
								param:{
									...
									isdeposit:$("#pay_type").data("kendoDropDownList").value(),			//支付方式
									sort:sort
								}
							};
							console.log('查询参数',parameter);
...
//						template :function(e)
//						{
//							if(e.price*e.product_num != e.totalprice)
//							{
//								return "<p class='text-center' style='TEXT-DECORATION:line-through;margin-bottom: 0px;color:red;'>"+e.price*e.product_num+"</p></n>"+
//									   "<p class='text-center'>"+e.totalprice+"</p>"
//							}else
//							{
//								return  "<p class='text-center'>"+e.price*e.product_num+"</p>"
//							}
//						}
					},
					{
...
//						template : "<p class='text-center'>#=payState#</p>"    
					...
					},
					{
						...
						title : "操作",     
						width : "80px",
						template : "<div value='查看详情' class=' k-state-default operation  ' ng-click='showDetails(\"#: op_id #\")' />查看详情</div>"
					},
