App.controller("addBrandDoorProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
  ...
  //初始化图文编辑器
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/brand/getdoorbyid/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				//付款模式
				if(data.doorBaseEntity.sales_model==2){
					$scope.doorBaseEntity.sales_model=data.doorBaseEntity.sales_model;
					$scope.doorBaseEntity.percent = data.doorBaseEntity.percent;
				}else{
					$scope.doorBaseEntity.sales_model=data.doorBaseEntity.sales_model;
				}
				//颜色分类
				for (var l = 0; l < data.doorSalesEntities.length; l++) {
					...
					$("#deleteHtml"+l).find("input[name='ClassifHiddenId']").val(data.doorSalesEntities[l].id);
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#addBrandDoorProductCtrl").find("input").attr("readonly","readonly");
					$("#addBrandDoorProductCtrl").find("input").attr("disabled","disabled");
					$("#addBrandDoorProductCtrl").find("select").attr("readonly","readonly");
					$("#addBrandDoorProductCtrlButton").css("display","none");
	...
	$scope.saveWallpaper=function(){
		...
		$scope.doorBaseEntity.unit = $("#unit").data("kendoComboBox").value().toString();//产品单位
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/brand/modifydoor",
				method:'POST', 
				...
				if(data.code==0){
					alert("修改成功");
					$location.path("/brandDoorProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
		...
		html='<div class="col-sm-12 classifClass" style="height:110px;" id="deleteHtml'+ti+'" title="'+ti+'">'+
			...
			'<div class="col-md-12" style="padding:0">'+
				'<div class="col-xs-8" style="padding:0">'+
	 			 ...
	 			 '</div>'+
	 			 '<div class="col-xs-2">'+
	 			 	'<button id="deleteColourClassify'+ti+'" ng-click="deleteColourClassify('+ti+')" class="k-button">删除</button>'+
	 			 '</div>'+
			'</div>'+
		'</div>';
