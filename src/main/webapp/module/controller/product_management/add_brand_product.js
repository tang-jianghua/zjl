App.controller("addBrandProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
	$scope.form = {};                  //表单
	$scope.baseEntity = {}             
  ...
  //初始化图文编辑器
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/brand/wallpaperbyid/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				$("#comb_position").data("kendoComboBox").value(data.baseEntity.haspicture);
				...
				$("#comb_newProduct").data("kendoComboBox").value(data.isNew);
				//判断回显数据是否是新品 非新品时间框不可用
				if(parseInt(data.isNew)==2){
					$("#comb_start_time").data("kendoDatePicker").value("");
            		$("#comb_start_time").data("kendoDatePicker").readonly(true);
				}
				...
				for (var l = 0; l < data.wallpaperSalesEntities.length; l++) {
					...
					$("#deleteHtml"+l).find("input[name='ClassifHiddenId']").val(data.wallpaperSalesEntities[l].id);
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#addBrandProductCtrl").find("input").attr("readonly","readonly");
					$("#addBrandProductCtrl").find("input").attr("disabled","disabled");
					$("#addBrandProductCtrl").find("select").attr("readonly","readonly");
					$("#addBrandProductCtrlButton").css("display","none");
	...
	$scope.saveWallpaper=function(){
		...
		//$scope.baseEntity.sales_model=$("input[name='marketClass']").val();// 付款模式
		...
		$scope.baseEntity.haspicture = $("#comb_position").data("kendoComboBox").value().toString();//是否图案
		alert($scope.baseEntity.haspicture);
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/brand/modifywallpaper",
				method:'POST', 
				...
				if(data.code==0){
					alert("修改成功");
					$location.path("/brandWallpaperProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
			...
			$http({
				url: path+"/server/brand/addwallpaper",
				method:'POST', 
				data:angular.toJson($scope.form),
				headers:  {
					"X-Object-Header" : "123456789 ",
		...
		html='<div class="col-sm-12 classifClass" style="height:110px;" id="deleteHtml'+ti+'" title="'+ti+'">'+
			...
			'<div class="col-md-12">'+
				'<div class="col-xs-7">'+
	 			 ...
	 			 '</div>'+
	 			 '<div class="col-xs-2">'+
	 			 	'<button ng-click="deleteColourClassify('+ti+')" id="deleteColourClassify'+ti+'" class="k-button">删除</button>'+
	 			 '</div>'+
			'</div>'+
		'</div>';
		html = $compile(html)($scope);				
