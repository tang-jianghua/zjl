App.controller("addCoatingProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
  ...
  //加载所属分类
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/brand/getpaintbyid/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				$scope.paintBaseEntity.sales_model=data.paintBaseEntity.sales_model;
				$scope.paintBaseEntity.spray_type=data.paintBaseEntity.spray_type;
				...
				//付款模式
				if(data.paintBaseEntity.sales_model==2){
					$scope.paintBaseEntity.sales_model=data.paintBaseEntity.sales_model;
					$scope.paintBaseEntity.percent=data.paintBaseEntity.percent;
				}else{
					$scope.paintBaseEntity.sales_model=data.paintBaseEntity.sales_model;
				}
				//颜色分类
				for (var j = 0; j < data.paintSalesEntities.length; j++) {
					...
					$("#deleteHtml"+j).find("input[name='ClassifHiddenId']").val(data.paintSalesEntities[j].id);
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#addCoatingProductCtrl").find("input").attr("readonly","readonly");
					$("#addCoatingProductCtrl").find("input").attr("disabled","disabled");
					$("#addCoatingProductCtrl").find("select").attr("readonly","readonly");
					$("#addCoatingProductCtrlButton").css("display","none");
	...
	$scope.saveTlie=function(){
		...
		$scope.paintBaseEntity.unit = $("#unit").data("kendoComboBox").value().toString();//产品单位
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/brand/modifypaint",
				method:'POST', 
				...
				if(data.code==0){
					alert("修改成功");
					$location.path("/brandCoatingProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
			...
			$http({
				url: path+"/server/brand/addpaint",
				method:'POST', 
				data:angular.toJson($scope.form),
				headers:  {
					"X-Object-Header" : "123456789 ",
