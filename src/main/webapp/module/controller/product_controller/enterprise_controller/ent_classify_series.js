App.controller("entproductClassifySeries",function($scope, $rootScope, $location, $http, $compile, path){
	...
	function beforeDrop(treeId, treeNodes, targetNode, moveType){
		if(treeNodes[0].level!=0){
			...
			for (var i = 0; i < treeNodesChildren.length; i++) {
				$http({
					   url: path+"/server/ent/modifyseries",
					   method: 'POST',   
		...
		$("#popover_" +treeNode.id+ "_i1").bind("click", function(){
			if(confirm("是否删除 " + treeNode.name + "？")){
				if(treeNode.parent_id==0){
					$http({
					   url: path+"/server/ent/removeclassify/"+treeNode.id,
					   method: 'GET',
					}).success(function(data){
						console.log(data);
						if(data.code==0){
					...
					$http({
						   url: path+"/server/ent/removeseries/"+treeNode.id,
						   method: 'GET',
						}).success(function(data){
							console.log(data);
							if(data.code==0){
	...
	function zTreeBeforeRename(treeId, treeNode, newName, isCancel){
		...
		if(treeNode.id==""){
			...
			if(treeNode.parent_id==0){
				$http({
				   url: path+"/server/ent/createclassify",
				   method: 'POST',
				...
				$http({
				   url: path+"/server/ent/createseries",
				   method: 'POST',
