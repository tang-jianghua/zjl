/*添加页面主controller*/
App.controller("addTrademarkProduct",function($scope, $rootScope, $location, $http, $compile, $stateParams, path, $timeout) {
	$scope.data=[];
	$scope.from={};
	...
	var editor = UE.getEditor('containerp');
	if(editor.container!=undefined){
		editor.destroy();
		editor = UE.getEditor('containerp');
	}
	//判断是线上或线下实体
	if(params.jumpsUrlType==1){
		$("#jumpsUrl").css("display","block");
	}else{
		$("#jumpsUrl").css("display","none");
	}
	...
	$scope.appendImg=function(e){
		html='<div class="col-sm-2 caseImg">'+
			...
			'</div>';
		$("#uplodingImgController1").append(html);
		$scope.Upload1("caseUpload"+e,"caseUpload_img"+e,"caseUpload_url"+e,150,150,150,"jpg");
	...
	$scope.addColourClassify = function(){
		...
		$scope.Upload("uploadColourClassify"+len,"ColourClassify_img"+len,"ColourClassify_url"+len,100,100,20,"jpg");	
	}
	//添加一条规格框
	...
	$scope.geteditData=function(){
		$http({
			...
			if(data!=null && data!="" && data!=undefined){
				...
				console.log(data.brandProdAttrValueEntities);
				//筛选条件回显
				var value=new Array();
				...
				if(params.jumpsUrlType==1){
					$("#jumpsUrlInput").val(data.sell_url);
				}
	...
	$scope.verification=function(){
		if(params.jumpsUrlType==1){
			if($scope.isNull($("#jumpsUrlInput").val())){
				alert("跳转路径不可为空");
				return false;
			}
		}
	...
	$scope.saveWallpaper=function(){
		...
		if($scope.isNull($scope.from.product_title)){
			...
			if(params.jumpsUrlType==1){
				$scope.from.sell_url=$("#jumpsUrlInput").val();
			}
			...
			if(params.method=="edit"){
				if(confirm("您确定修改该产品吗？")){
					$http({
						url: path+"/server/modfiynewproduct",
						method:'POST', 
						...
						if(data.code==0){
							alert("修改成功");
							if(params.is==1){
								$location.path("/trademarkUpshelfProduct");
							}else if(params.is==2){
								$location.path("/trademarkSoldoutProduct");
							}else if(params.is==3){
								$location.path("/trademarkNewProduct");
							}else if(params.is==4){
								$location.path("/trademarkBrandProduct");
							}
						}else{
        	...
        	for (var s = 0; s < StandardArray.length; s++) {
        		...
        		//删除
                 if(color_id!=null&&color_id!=""&&color_id!=undefined){
      		        $http({
      					url: path+"/server/removebrandprodsales/"+color_id,
      					method:'GET', 
      					headers:  {
      						"X-Object-Header" : "123456789 ",
      			            "Content-Type": "application/json"
	...
	$scope.deleteStandareData=function(id,len){
		var color_id=$("#standard"+id+len).attr("colorid");
		 if(color_id!=null&&color_id!=""&&color_id!=undefined){
		        $http({
					url: path+"/server/removebrandprodsales/"+color_id,
					method:'GET', 
