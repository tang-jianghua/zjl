App.controller("entaddBrandFloorProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
	...
	$scope.init("floor_unit","wallpaperunit","单位");
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/ent/getfloorbyid/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				$scope.floorSalesEntities.isline=data.floorSalesEntities[0].isline;
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#entaddBrandFloorProductCtrl").find("input").attr("readonly","readonly");
					$("#entaddBrandFloorProductCtrl").find("input").attr("disabled","disabled");
					$("#entaddBrandFloorProductCtrl").find("select").attr("readonly","readonly");
					$("#entaddBrandFloorProductCtrlButton").css("display","none");
					...
					$("#floor_start_time").data("kendoDateTimePicker").readonly(true);
				}
	...
	$scope.saveFloorWallpaper=function(){
		...
		for (var i = 0; i < colClass.length; i++) {
			var json={
					"isline":$scope.floorSalesEntities.isline,
					"model":pm[i].value,
					"color":an[i].value,
					"price":ap[i].value,
					"stock":ar[i].value,
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/ent/modifyfloor",
				method:'POST', 
				...
				if(data.code==0){
					alert("修改成功");
					$location.path("/entbrandFloorProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
