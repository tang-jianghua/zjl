App.controller("warehouseProduct1",function($scope, $rootScope, $location, $http, $compile,path) {
	$scope.form={};
	var num=0;
	...
	var operation={ title : "操作",
					template:function(e){
						var template_html = '<div class="operation k-state-default" ng-click="edit('+e.product_id+')">编辑</div>'
						    +'<div class="operation k-state-default" ng-click="copyUrl(\''+e.id+'\')">复制链接</div>';
						return template_html;}
				    };
	var operation1={ title : "操作",
			template:function(e){
				var template_html = '<div class="operation k-state-default" ng-click="edit(\''+e.fb_product_id+'\')">编辑</div>'
				+'<div class="operation k-state-default" ng-click="copyUrl(\''+e.id+'\')">复制链接</div>';
				return template_html;}
	};
	var operation2={ title : "操作",
		template:function(e){
    		var template_html = '<div class="operation k-state-default" ng-click="edit(\''+e.db_product_id+'\')">编辑</div>'
			    +'<div class="operation k-state-default" ng-click="copyUrl(\''+e.id+'\')">复制链接</div>';
    		return template_html;
        }
	};
	var operation3={ title : "操作",
			template:function(e){
				var template_html = '<div class="operation k-state-default" ng-click="edit(\''+e.id+'\')">编辑</div>'
				+'<div class="operation k-state-default" ng-click="copyUrl(\''+e.id+'\')">复制链接</div>';
				return template_html;
			}
	};
