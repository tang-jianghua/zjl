App.controller("entaddSwitchProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
  ...
  //加载所属分类
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/ent/getsaleattr/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#entaddSwitchProductCtrl").find("input").attr("readonly","readonly");
					$("#entaddSwitchProductCtrl").find("input").attr("disabled","disabled");
					$("#entaddSwitchProductCtrl").find("select").attr("readonly","readonly");
					$("#entaddSwitchProductCtrlButton").css("display","none");
	...
	$scope.saveTlie=function(){
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/ent/modifysaleattr",
				method:'POST', 
				...
				if(data.code==0){
					alert("修改成功");
					$location.path("/entbrandSwitchProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
			...
			$http({
				url: path+"/server/ent/addsaleattr",
				method:'POST', 
				data:angular.toJson($scope.form),
				headers:  {
					"X-Object-Header" : "123456789 ",
	...
	$scope.addColourClassify=function(){
		...
		ti=parseInt(ti)+1;
		html='<div class="col-sm-11 classifClass" style="height:110px;" id="deleteHtml'+ti+'" title="'+ti+'">'+								
			'<div class="col-xs-7">'+
