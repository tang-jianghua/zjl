App.controller("entaddTileProductCtrl",function($scope, $rootScope, $location, $http, $compile, $routeParams, path) {
  ...
  //加载所属分类
	...
	if(params.method=="edit"||params.method=="check"){
		...
		$http({
			url: path+"/server/ent/gettilebyid/"+params.id,
			method:'GET', 
			headers:  {
			...
			if(data!=null&&data!=""&&data!=undefined){
				...
				//设置只读不可编辑
				if(params.method=="check"){
					$("#entaddTileProductCtrl").find("input").attr("readonly","readonly");
					$("#entaddTileProductCtrl").find("input").attr("disabled","disabled");
					$("#entaddTileProductCtrl").find("select").attr("readonly","readonly");
					$("#entaddTileProductCtrlButton").css("display","none");
	...
	$scope.saveTlie=function(){
		...
		if(params.method=="edit"){
			$http({
				url: path+"/server/ent/modifytile",
				method:'POST', 
				...
				if(data.code=="0"){
					alert("修改成功");
					$location.path("/entbrandTileProduct/{}");
				}else{
					alert("修改失败");
				}
			}).error(function(data){
