App.controller("brandBookingForBrandCtrl",function($scope, $rootScope, $location, $http, $compile, path, $sce, publicService) {
	...
	$scope.mainGridOptions = {
				...
				schema : {
					...
					model: {
				          ...
				          fields: {
				        	 ...
				        	 // shop_name: {defaultValue: { id: 2, shop_name: "龍冉壁纸"} },
			...
			columns : [
					...
					},
					{
						...
						//template:'<div class="format"><img style="width: 100%;margin:auto;" src="#: door_model_img #" /></div>'
					},
					{
						field : "name",         
						title : "姓名",     
					...
					},
					{
						...
						template: function(e){
					    	var className = '';
					    	if(e.state==1 || e.state==6){  //已提交,无法受理   
					    		className = 'canEdit';
					    	}else{
					    		className = 'noEdit';
					    	}
					    	...
					    	if(e.shop_name){
					    		return '<div class="'+className+'">'+e.shop_name+'</div>';
					    	}else{
					    		return '';
					    	}
					    },
						editor: function (container, options) { 
				    	    ...
				    	    if(state==1 || state==6){ //已提交,无法受理
				    	    	$('<input data-bind="value:' + options.field + '"/>')
						        .appendTo(container)
						        .kendoDropDownList({
						           dataTextField: "shop_name",
								   dataValueField: "id",  //id
						           dataSource: {
						 	          transport: {
						 	              read: {
						 	            	  type : 'GET',
						 	                  url: path+"/server/queryshopname",
						 	                  dataType : "json"
						 	              }
						 	          },
						 	      },
						 	      change: function(e) {
						 	        ...
						 	        $scope.editShopName(options.model.id,value);
						 	      }
						        });
				    	    }else{
				    	    	if(options.model.shop_name){
				    	    		$('<span>'+options.model.shop_name+'</span>')
							        .appendTo(container);
				    	    	}
				    	    }
	...
	//编辑店面名称
	$scope.editShopName = function(id,shopId){
		var postData = {
				id: id,
				shopId: shopId
		};
		...
		$http({
			   url: path+"/server/modfiyappointmentinfoshop",
			   method: 'POST',
